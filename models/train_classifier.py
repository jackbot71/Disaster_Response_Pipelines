...
def main():
    if len(sys.argv) == 4:
        database_filepath, database_tablename, model_filepath = sys.argv[1:]
        print('Loading data...\n    DATABASE: {}'.format(database_filepath))
        X, Y = load_data(database_filepath, database_tablename)
        ...
        print('Please provide the filepath of the disaster messages database '\
              'as the first argument, the table name as the second and the filepath of the pickle file to '\
              'save the model to as the third argument. \n\nExample: python '\
              'train_classifier.py ../data/DisasterResponse.db msg_df classifier.pkl')
